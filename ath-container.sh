...
docker run $run_opts $run_drive_mapping $tag /bin/bash -c "set-java.sh $java_version; vnc.sh ; export DISPLAY=:42; run.sh firefox latest -DforkCount=1 -B -Dtest=WarningsNextGenerationPluginTest"
