...
export DISPLAY=:99.0 && docker run $run_opts $run_drive_mapping $tag /bin/bash -c "set-java.sh $java_version; export DISPLAY=:99.0; export BROWSER_DISPLAY=:99.0; run.sh firefox latest -Dmaven.test.failure.ignore=true -DforkCount=1 -B -Dtest=WarningsNextGenerationPluginTest#should_expand_token"
